...
@NamedQueries({
    ...
    @NamedQuery(name = "SpecificBean.findByAccountSpecific", query = "SELECT s FROM SpecificBean s "
            + "WHERE s.dictatorworktypeid is NULL"),
    @NamedQuery(name = "SpecificBean.findByDictator&WorkType", query = "SELECT d FROM SpecificBean d "
            + "WHERE (d.dictatorworktypeid.dictatorid.dictatorid = :dictatorid AND "
            + "d.dictatorworktypeid.worktypeid.worktype LIKE :worktype)")})
